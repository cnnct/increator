...
> > **id ：**div的id属性
> >
> > **size ：** size为尺寸标签,可以填的数值范围为（1-12）,如size="6",当type为label时，默认size为2，当type为value时，默认为10。每一列实际宽度为该列的最大宽度\(如：总共两行，第一行第一列size为5，第二行第一列size为3，那么每行第一列size都为5\)，有点类似html中的table 。注意：如果一行的总size大于或者小于12，那么可能达不到预期的效果，所以不要这么做。
> >
